...
ko.validation.rules['imgsize'] = {
    validator: function (val,fields) {
        var file =val;
        var widthField = fields[0];
        var heightField =fields[1];
        img = new Image();
        img.src=file;
        if(img.width <= widthField || img.height <= heightField){
            return false;
        }
        return true;
    },
    message: 'Please upload image with and height should be greater than {0}X{1}'
};
